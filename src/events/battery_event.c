...
static void log_battery_state_event(const struct app_event_header *aeh)
{
	...
	BUILD_ASSERT(ARRAY_SIZE(state_name) == BATTERY_STATE_COUNT, "Invalid number of elements");
...
APP_EVENT_TYPE_DEFINE(battery_state_event,
					  ...
					  APP_EVENT_FLAGS_CREATE(
						  IF_ENABLED(CONFIG_ISP_LOG_BATTERY_STATE_EVENT,
...
APP_EVENT_TYPE_DEFINE(battery_level_event,
					  ...
					  APP_EVENT_FLAGS_CREATE(
						  IF_ENABLED(CONFIG_ISP_LOG_BATTERY_LEVEL_EVENT,
									 (APP_EVENT_TYPE_FLAGS_INIT_LOG_ENABLE))));
